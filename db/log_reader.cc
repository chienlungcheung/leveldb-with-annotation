...
bool Reader::ReadRecord(Slice* record, std::string* scratch) {
  ...
  while (true) {
    ...
    if (resyncing_) {
      // 只要数据没有损害或者到了文件尾, 而且返回的 record_type 只要不是 kBadRecord(返回该类型其中一个情况就是起始地址不满足条件)
      // 就说明当前 record 起始地址已经大于 initial_offset_ 了,
      // 但是由于 type 为 middle 或者 last, 所以逻辑上这个 record 仍然与不符合 initial_offset_ 的
      // 类型为 first 的 record 是一家的, 所以当前 record 也不是我们要的.
      if (record_type == kMiddleType) {
        ...
        // 如果是 full 类型的 record, 而且这个 record 起始地址大于 inital_offset_, 满足条件了, 关掉标识.
        // TODO 如果返回 kBadRecord 或未知类型, 这里也会关掉该标识, 不太懂为什么?
        resyncing_ = false;
      }
    }
    ...
    switch (record_type) {
      ...
      case kMiddleType:
        if (!in_fragmented_record) { // 都存在 MIDDLE 了, 竟然还说当前 record 没分片, 报错. 
...
unsigned int Reader::ReadPhysicalRecord(Slice* result) {
  // 循环直到获取到满足条件(record 起始地址不小于 initial_offset_)的 record 或者失败为止
  while (true) {
    // buffer_ 底层指针会向前移动, 所以其 size 是动态的. 
    // 如果小于 kHeaderSize 且不为空, 表示 buffer 里面剩余字节是个 trailer, 可以跳过它去解析下个 block 了. 
    if (buffer_.size() < kHeaderSize) {
      if (!eof_) { // 如果未到文件尾
        ...
        // 更新 end_of_buffer_offset_ 至迄今读取最大位置下一个字节
        end_of_buffer_offset_ += buffer_.size(); 
        // 如果 log 文件损坏
        if (!status.ok()) {
          buffer_.clear();
          ReportDrop(kBlockSize, status);
          // 读文件失败我们认为到达文件尾; 注意, file_->Read 读到文件尾也不会报错因为这是正常情况, 只有遇到错误才会报错. 
          eof_ = true; 
          return kEof;
    ...
    // 如果解析出的 length 加上固定 header 长度大于 buffer_ 剩余数据长度, 则说明数据损坏了, 比如 length 被篡改了. 
