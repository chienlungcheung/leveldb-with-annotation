...
// 将 slice 中保存的 record 追加到 log 文件中
Status Writer::AddRecord(const Slice& slice) {
  ...
  do {
    ...
    assert(leftover >= 0);
    // 如果当前 block 剩余空间不足容纳 record 的 header(7 字节) 则剩余空间作为 trailer 填充 0
...
Status Writer::EmitPhysicalRecord(RecordType t, const char* ptr, size_t n) {
  ...
  // 将 n 编码到 length 字段, little endian
  ...
  // 当前 block 剩余空间起始偏移量. 注意, 这里不管 header 和 data 是否写成功. 
